...
import { saveCar } from "@/api/car/car";
import { setShopCarInfo } from "@/utils/common";
/*
 * @Description: 购物车状态模块
 */
export default {
  state: {
    shoppingCart: getShopCarInfo(),
    // shoppingCart结构
    /* 
    shoppingCart = {
      id: "", // 购物车id
  ...
  actions: {
    setShoppingCart({ commit }, data) {
      if (data.type && data.type == 1) {
        saveCar(data.saveResult)
          .then((res) => {
            if (res.code == 200) {
              // 200 为成功, 更新vuex购物车状态
              commit("setShoppingCart", res.data.shoppingCartData);
